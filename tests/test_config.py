...
def test_defaults(monkeypatch):
    for var in ("NETAUDIO_GUI_BIND", "NETAUDIO_GUI_PORT", "NETAUDIO_GUI_USERS",
    ...
    assert s.users_path.endswith("netaudio-webgui/users.json")
...
def test_users_path_override(monkeypatch):
    monkeypatch.setenv("NETAUDIO_GUI_USERS", "/tmp/users.json")
    assert load_settings().users_path == "/tmp/users.json"
